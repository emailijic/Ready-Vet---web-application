...
export class AllUsersComponent implements OnInit {
  ...
  getAnimals() {
    this.animalService.getAllAnimals({ ...this.whereAnimal })
      .subscribe(res => {
        this.petsTotal = res.total;
        this.animals = res.data;
      });
  }

  getAnimalOfPetOwner(id: string): number {
    let total = 0;
    this.animals.forEach(el => {
      if (get(el, '_petOwner._id') === id) {
        total += 1;
      }
    });
    return total;
